...
<style>
  ...
  .auth-container {
    ...
    background-size: cover; background-position: center; padding: 2rem;
  }
  ...
  .password-toggle {
    ...
    color: var(--gray-color); transition: all var(--transition-speed) ease;
  }
  ...
  .auth-btn {
    width: 100%; height: 50px;
    background-color: #e94f1b; color: white;
  ...
  .auth-btn:hover { background-color: #fc9700; transform: translateY(-2px); }
  .auth-btn:active { transform: translateY(0); }

  .error-message {
    color: var(--error-color); font-size: 0.85rem; margin-top: 0.5rem;
  ...
  @media (max-width: 576px) {
    ...
    .auth-header, .auth-body { padding: 1.5rem; }
  }
</style>
</head>
<body>
<div class="auth-container">
  <div class="auth-card animate__animated animate__fadeIn">
    <div class="auth-header">
      <img src="{{ asset('assetsPoster/assets/images/logo_car225.png') }}" class="auth-logo floating" alt="Logo" style="background-color: white">
      <h1 class="auth-title">Finalisation de l'inscription</h1>
      <p class="auth-subtitle">Complétez les informations pour finaliser votre compte gare</p>
    </div>

    <div class="auth-body">
      <form method="POST" action="{{ route('gare-espace.validate', $email) }}">
        @csrf
        ...
        <!-- Code (hidden - auto-filled after OTP verification) -->
        <input type="hidden" name="code" value="{{ $code }}">

        <!-- Password -->
        <div class="input-group">
          <i class="fas fa-key input-icon"></i>
          <input class="input-field" type="password" name="password" id="password" placeholder="Mot de passe (min. 8 caractères)" required>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    @if(Session::has('success'))
      Swal.fire({ icon: 'success', title: 'Succès', text: '{{ Session::get('success') }}', confirmButtonColor: '#e94f1b' });
    @endif
