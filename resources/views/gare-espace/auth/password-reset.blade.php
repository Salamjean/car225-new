...
<head>
    ...
    <title>Réinitialisation du mot de passe Gare - Car 225</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .reset-header { background: linear-gradient(to right, #343a40, #495057); color: var(--white); padding: 30px; text-align: center; }
        ...
        .btn-primary { background: linear-gradient(to right, var(--primary), #ffb74d); color: var(--white); }
...
<body>
    <div class="reset-container">
        <div class="reset-card">
            <div class="reset-header">
                <h1><i class="fas fa-warehouse"></i> Accès Gare</h1>
                <p>Réinitialisation du compte gare</p>
            </div>
                ...
                <div class="step-content active" id="step-1">
                    <form id="emailForm">
                        <div class="form-group">
                            <label for="email">Email Gare</label>
                    ...
                    <div class="back-to-login"><a href="{{ route('gare-espace.login') }}"><i class="fas fa-arrow-left"></i> Retour à la connexion</a></div>
                </div>
    ...
    <script>
        ...
        document.getElementById('emailForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('{{ route("gare-espace.password.sendOtp") }}', {
                    method: 'POST', headers: { 
        ...
        document.getElementById('otpForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('{{ route("gare-espace.password.verifyOtp") }}', {
                    method: 'POST', headers: { 
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}' 
        ...
        document.getElementById('passwordForm').addEventListener('submit', async function(e) {
            ...
            if (password !== passwordConfirmation) { Swal.fire({ icon: 'error', text: 'Les mots de passe ne correspondent pas.' }); return; }
            const btn = document.getElementById('resetPasswordBtn'); btn.disabled = true;
            try {
                const response = await fetch('{{ route("gare-espace.password.reset") }}', {
                    method: 'POST', headers: { 
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}' 
                ...
                const data = await response.json();
                if (data.success) { Swal.fire({ icon: 'success', text: data.message }).then(() => { window.location.href = '{{ route("gare-espace.login") }}'; }); }
                else { Swal.fire({ icon: 'error', text: data.message }); }
