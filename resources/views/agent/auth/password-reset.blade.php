...
<head>
    ...
    <title>Réinitialisation du mot de passe Agent - Car 225</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            ...
            min-height: 100vh;
            margin: 0;
            background: linear-gradient(rgba(255, 255, 255, 0.1), rgba(0,0,0, 0.8)), url('{{ asset('assets/assets/img/arrierep.jpg') }}');
...
<body>
    <div class="reset-container">
        <div class="reset-card">
            <div class="reset-header">
                <h1><i class="fas fa-user-shield"></i> Accès Agent</h1>
                <p>Réinitialisation du compte agent</p>
            </div>
            ...
            <div class="reset-body">
                <div class="step-content active" id="step-1">
                    ...
                    <div class="back-to-login"><a href="{{ route('agent.login') }}"><i class="fas fa-arrow-left"></i> Retour</a></div>
                </div>
    ...
    <script>
        ...
        document.getElementById('emailForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('{{ route("agent.password.sendOtp") }}', {
                    method: 'POST', headers: { 
        ...
        document.getElementById('otpForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('{{ route("agent.password.verifyOtp") }}', {
                    method: 'POST', headers: { 
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}' 
        ...
        document.getElementById('passwordForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('{{ route("agent.password.reset") }}', {
                    method: 'POST', headers: { 
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}' 
                ...
                const data = await response.json();
                if (data.success) { Swal.fire({ icon: 'success', text: data.message }).then(() => { window.location.href = '{{ route("agent.login") }}'; }); }
                else { Swal.fire({ icon: 'error', text: data.message }); }
