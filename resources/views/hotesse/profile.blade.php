        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white rounded-3xl shadow-xl p-8">
                    <div class="text-center">
                        @if($hotesse->profile_picture)
                            <img src="{{ asset('storage/' . $hotesse->profile_picture) }}" 
                        ...
                        @else
                            <div class="w-40 h-40 rounded-full mx-auto mb-4 border-4 border-[#e94e1a] bg-gradient-to-r from-[#e94e1a] to-[#d33d0f] flex items-center justify-center">
                                <span class="text-6xl font-bold text-white">
                                    {{ strtoupper(substr($hotesse->prenom, 0, 1)) }}{{ strtoupper(substr($hotesse->name, 0, 1)) }}
                                </span>
                            </div>
                        @endif
                        
                        <h3 class="text-2xl font-bold text-gray-900">{{ $hotesse->prenom }} {{ $hotesse->name }}</h3>
                        <p class="text-gray-600">{{ $hotesse->email }}</p>
                        
                        <div class="mt-6 space-y-3">
                            <div class="flex items-center justify-between p-3 bg-orange-50 rounded-lg">
                                <span class="text-sm text-gray-600">Tickets</span>
                                <span class="font-bold text-[#e94e1a]">{{ $hotesse->tickets }}</span>
                            </div>
                            <div class="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                                <span class="text-sm text-gray-600">Statut</span>
                                <span class="font-bold text-green-600">{{ $hotesse->isArchived() ? 'Archivé' : 'Actif' }}</span>
                            </div>
                            <div class="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                                <span class="text-sm text-gray-600">Code ID</span>
                                <span class="font-bold text-gray-800">{{ $hotesse->code_id ?? 'N/A' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-3xl shadow-xl p-8">
                    ...
                    <form action="{{ route('hotesse.profile.update') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Nom *</label>
                                <input type="text" name="name" value="{{ old('name', $hotesse->name) }}" required
                            ...
                            <div>
                                ...
                                <input type="text" name="prenom" value="{{ old('prenom', $hotesse->prenom) }}" required
                                    class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#e94e1a] focus:border-transparent">
                                @error('prenom')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="text" name="contact" value="{{ old('contact', $hotesse->contact) }}" required
                                    class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#e94e1a] focus:border-transparent">
                                @error('contact')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="text" name="cas_urgence" value="{{ old('cas_urgence', $hotesse->cas_urgence) }}"
                                    class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#e94e1a] focus:border-transparent">
                                @error('cas_urgence')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="commune" value="{{ old('commune', $hotesse->commune) }}"
                                    class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#e94e1a] focus:border-transparent">
                                @error('commune')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                ...
                <div class="bg-white rounded-3xl shadow-xl p-8">
                    ...
                    <form action="{{ route('hotesse.profile.password') }}" method="POST">
                        @csrf
