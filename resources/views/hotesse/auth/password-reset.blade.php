...
<head>
    ...
    <title>Réinitialisation du mot de passe Hôtesse - Car 225</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            ...
            background: linear-gradient(rgba(255, 255, 255, 0.1), rgba(233, 79, 27, 0.8)), url('{{ asset('assets/assets/img/arrierep.jpg') }}');
        ...
        .reset-header { background: linear-gradient(to right, #e94f1b, #ffb74d); color: var(--white); padding: 30px; text-align: center; }
        ...
        .step-item.active .step-number { background: #e94f1b; color: var(--white); }
        ...
        .btn-primary { background: linear-gradient(to right, #e94f1b, #ffb74d); color: var(--white); }
        ...
        .back-to-login a { color: #e94f1b; text-decoration: none; font-size: 14px; font-weight: 600; }
...
<body>
    <div class="reset-container">
        <div class="reset-card">
            <div class="reset-header">
                <h1><i class="fas fa-concierge-bell"></i> Accès Hôtesse</h1>
                <p>Réinitialisation du compte hôtesse</p>
            </div>
                ...
                <div class="step-content active" id="step-1">
                    <form id="emailForm">
                        <div class="form-group">
                            <label for="email">Email Hôtesse</label>
                    ...
                    <div class="back-to-login"><a href="{{ route('hotesse.auth.login') }}"><i class="fas fa-arrow-left"></i> Retour</a></div>
                </div>
    ...
    <script>
        ...
        document.getElementById('emailForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('{{ route("hotesse.password.sendOtp") }}', {
                    method: 'POST', headers: { 
        ...
        document.getElementById('otpForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('{{ route("hotesse.password.verifyOtp") }}', {
                    method: 'POST', headers: { 
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}' 
        ...
        document.getElementById('passwordForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('{{ route("hotesse.password.reset") }}', {
                    method: 'POST', headers: { 
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}' 
                ...
                const data = await response.json();
                if (data.success) { Swal.fire({ icon: 'success', text: data.message }).then(() => { window.location.href = '{{ route("hotesse.auth.login") }}'; }); }
                else { Swal.fire({ icon: 'error', text: data.message }); }
