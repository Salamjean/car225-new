...
@section('content')
    <div class="min-h-screen bg-gradient-to-br from-white to-blue-50 py-4 sm:py-6 lg:py-8">
                                ...
                                <div class="block md:hidden">
                                    <div class="p-4 space-y-4">
                                        ...
                                        <div class="flex items-start gap-3 pb-3 border-b border-gray-100">
                                            <div class="w-12 h-12 bg-gradient-to-r from-[#fea219] to-orange-500 rounded-full flex items-center justify-center flex-shrink-0">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <div class="flex items-center gap-2 mb-1">
                                                    ...
                                                    @if($programme->is_aller_retour)
                                                        <span class="bg-blue-100 text-blue-800 text-xs px-2 py-0.5 rounded-full font-semibold whitespace-nowrap">
                                        ...
                                        <div class="flex items-center justify-center">
                                            <span class="px-3 py-1 rounded-full text-sm font-semibold border {{ $statusColors[$statut] ?? 'bg-gray-100 text-gray-600 border-gray-200' }}">
                                        ...
                                        <div class="flex gap-2">
                                            ...
                                            @endif
                                            <button
                                                onclick="showVehicleDetails({{ $programme->vehicule_id ?? 'null' }}, '{{ $searchParams['date_depart'] ?? date('Y-m-d') }}')"
                                    ...
                                    <div class="grid grid-cols-12 gap-4 p-6 items-center">
                                        ...
                                        <div class="col-span-3">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <div class="flex items-center gap-2 mb-1" style="display:flex; justify-content:center">
                                                        <h3 class="font-bold text-gray-900 truncate">
                                                            {{ $programme->compagnie->name ?? 'Compagnie' }}
                                                        </h3>
                                                        @if($programme->is_aller_retour)
                                                        ...
                                                        @if($programme->compagnie)
                                                            <span class="bg-green-100 text-green-800 text-xs px-2 py-0.5 rounded-full font-semibold">
                                                                <i class="fas fa-check-circle text-xs"></i> Vérifié
                                                            </span>
                                                        @endif
                                                    </div>
                                                    <div class="flex items-center gap-2 text-sm text-gray-600" style="display:flex; justify-content:center">
                                        ...
                                        <div class="col-span-2" style="display:flex; justify-content:center">
                                            <span class="px-3 py-1 rounded-full text-sm font-semibold border {{ $statusColors[$statut] ?? 'bg-gray-100 text-gray-600 border-gray-200' }}">
                                                ...
                                                @if ($programme->type_programmation == 'recurrent')
                                                    <br><small class="text-xs">(pour le {{ date('d/m/Y', strtotime($searchParams['date_depart'] ?? now())) }})</small>
                                                @endif
                                            </span>
                                        </div>
        ...
        <script>
            ...
            // ============================================
            async function showVehicleDetails(vehicleId, searchDate = null) {
                if (!vehicleId) {
                    Swal.fire({
                        icon: 'info',
                        title: 'Information',
                ...
                // Récupérer la date depuis le formulaire de recherche
                let dateVoyage = searchDate;

                if (!dateVoyage) {
                    // Essayer de récupérer la date depuis l'input de recherche
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            <div class="flex flex-wrap gap-4">
                                ...
                                ${program.type_programmation === 'recurrent' ?
                                    `<span class="bg-green-100 text-green-800 px-2 py-1 rounded text-sm">
                                                                <i class="fas fa-redo-alt"></i> Jours: ${program.jours_recurrence ? JSON.parse(program.jours_recurrence).join(', ') : ''}
                                                            </span>` : ''}
                            </div>
                        `;

                            // IMPORTANT: Ajouter le paramètre date à l'URL
                            const seatsUrl =
                                `/user/booking/reservation/reserved-seats/${programId}?date=${encodeURIComponent(dateVoyage)}`;
                            console.log('Récupération places réservées pour date:', dateVoyage);
                            console.log('URL:', seatsUrl);

                            fetch(seatsUrl)
                                .then(response => response.json())
                                .then(seatData => {
                                    console.log('Réponse places réservées:', seatData);
                                    if (seatData.success) {
                                        reservedSeats = seatData.reservedSeats || [];
                                        console.log('Places réservées:', reservedSeats);
                                    }
                                })
                                .catch(error => {
