...
<div class="py-4 sm:py-8 px-3 sm:px-6 lg:px-8">
    <div class="mx-auto w-full max-w-4xl">
        ...
        <div class="mb-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-xl sm:text-3xl font-black text-[#1A1D1F] tracking-tight">Support Client</h1>
        ...
        {{-- Bannière WhatsApp --}}
        <div class="mt-8 bg-gradient-to-r from-[#25D366] to-[#128C7E] rounded-[32px] p-6 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-lg shadow-green-200">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-white font-black text-base">Besoin d'une aide immédiate ?</p>
                    <p class="text-white/80 text-sm font-medium">Discutez en direct avec notre équipe sur WhatsApp</p>
                </div>
            </div>
            @php
                $whatsappNumber = '2250798278981'; // ← Modifier avec le vrai numéro (format international sans +)
                $whatsappMsg = urlencode('Bonjour, j\'ai besoin d\'aide concernant ma réservation sur Car225.');
            @endphp
            <a href="https://wa.me/{{ $whatsappNumber }}?text={{ $whatsappMsg }}"
               target="_blank" rel="noopener noreferrer"
               class="flex-shrink-0 flex items-center gap-2.5 bg-white text-[#128C7E] px-6 py-3 rounded-2xl font-black text-sm hover:bg-green-50 transition-all duration-200 shadow-md hover:shadow-lg hover:-translate-y-0.5">
...
@php
    $waNumber = '2250708000000'; // ← Modifier avec le vrai numéro
    $waMsg = urlencode('Bonjour, j\'ai besoin d\'aide concernant ma réservation sur Car225.');
@endphp
<a href="https://wa.me/{{ $waNumber }}?text={{ $waMsg }}"
   target="_blank" rel="noopener noreferrer"
   id="waFloatingBtn"
   title="Contacter le support sur WhatsApp"
   class="fixed bottom-6 right-6 z-50 flex items-center gap-3 bg-[#25D366] text-white rounded-full shadow-2xl shadow-green-400/40 transition-all duration-300 hover:shadow-green-400/60 hover:-translate-y-1 group"
   style="padding: 14px 20px 14px 16px;">
    {{-- Icône WhatsApp --}}
    <div class="relative">
        <svg viewBox="0 0 24 24" class="w-7 h-7 fill-white flex-shrink-0">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
        </svg>
        {{-- Dot vert animé --}}
        <span class="absolute -top-0.5 -right-0.5 w-3 h-3 bg-white rounded-full flex items-center justify-center">
            <span class="w-2 h-2 bg-green-400 rounded-full animate-ping absolute"></span>
            <span class="w-2 h-2 bg-green-500 rounded-full relative"></span>
        </span>
    </div>
    {{-- Texte qui apparaît au hover --}}
    <span class="font-bold text-sm whitespace-nowrap max-w-0 overflow-hidden group-hover:max-w-xs transition-all duration-300 ease-in-out">
        Chat WhatsApp
    </span>
</a>
