...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-orange-50 py-8 px-4">
    <div class="mx-auto" style="width: 90%">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8">
            <div class="mb-6 lg:mb-0">
                ...
                <p class="text-lg text-gray-600">
                   Gérez vos caissières et rechargez leurs tickets
                </p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <!-- Total tickets en caisse -->
            <div class="bg-white rounded-2xl shadow-lg p-6 border-l-4 border-blue-500">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-600">Tickets en Caisse</p>
                        <p class="text-2xl font-bold text-gray-900 mt-1">{{ $caisses->sum('tickets') }}</p>
                    </div>
                ...
                <table class="w-full">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Tickets</th>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-200 caisse-row"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center justify-center space-x-3">
                                        <!-- Bouton Recharger -->
                                        <button type="button" onclick="showRechargeModal({{ $caisse->id }}, '{{ $caisse->prenom }} {{ $caisse->name }}', {{ $caisse->tickets }})"
...
function showRechargeModal(caisseId, caisseName, currentTickets) {
    Swal.fire({
        title: 'Recharger les tickets',
        html: `
            <div class="text-left space-y-4">
                <div class="bg-blue-50 p-4 rounded-lg">
                    <p class="text-sm font-medium text-gray-700">Caissière: <span class="text-blue-600">${caisseName}</span></p>
                    <p class="text-sm font-medium text-gray-700">Tickets actuels: <span class="text-blue-600">${currentTickets}</span></p>
                    <p class="text-sm font-medium text-gray-700">Vos tickets: <span class="text-green-600">{{ Auth::guard('compagnie')->user()->tickets }}</span></p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Nombre de tickets à recharger</label>
                    <input type="number" id="recharge-tickets" min="1" max="{{ Auth::guard('compagnie')->user()->tickets }}" 
        ...
        preConfirm: () => {
            ...
            if (tickets > {{ Auth::guard('compagnie')->user()->tickets }}) {
                Swal.showValidationMessage('Nombre de tickets insuffisant');
                return false;
            }
            return tickets;
        }
        ...
        if (result.isConfirmed) {
            rechargeTickets(caisseId, result.value);
        }
    });
}

// Recharger tickets via AJAX
function rechargeTickets(caisseId, tickets) {
    fetch(`/company/caisse/${caisseId}/recharge`, {
        method: 'POST',
        headers: {
