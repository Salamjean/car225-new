...
class PasswordResetController extends Controller
{
    ...
    public function sendOtp(Request $request)
    {
        ...
        if (!$caisse) {
            return response()->json([
                'success' => false,
                'message' => 'Aucun caissier n\'existe avec cette adresse email.'
            ]);
        }
    ...
    {
        $request->validate([
            'email' => 'required|email|exists:caisses,email',
        ...
        Caisse::where('email', $request->email)->update(['password' => Hash::make($request->password)]);
