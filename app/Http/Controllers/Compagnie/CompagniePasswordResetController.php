...
class CompagniePasswordResetController extends Controller
{
    /**
    ...
    {
        return view('compagnie.auth.password-reset');
    }
    ...
    public function sendOtp(Request $request)
    {
        ...
        $email = $request->email;
        $compagnie = Compagnie::where('email', $email)->first();
        if (!$compagnie) {
            return response()->json([
                'success' => false,
                'message' => 'Aucune compagnie n\'existe avec cette adresse email.'
            ]);
        }
        ...
        try {
            Mail::send('emails.otp', ['otp' => $otpCode], function ($message) use ($email) {
                $message->to($email)
                    ->subject('Code de réinitialisation de mot de passe Compagnie - Car225');
            });

            return response()->json([
                'success' => true,
                'message' => 'Un code OTP a été envoyé à l\'adresse email de la compagnie.',
                'email' => $email
            ]);
        } catch (\Exception $e) {
            return response()->json([
    ...
    {
        $request->validate([
            'email' => 'required|email|exists:compagnies,email',
            'otp' => 'required|string|size:6',
        ...
        return response()->json([
            'success' => true,
            'message' => 'Le mot de passe de la compagnie a été réinitialisé avec succès.'
        ]);
    }
}
