...
class PasswordResetController extends Controller
{
    ...
    {
        return view('gare-espace.auth.password-reset');
    }
    ...
    public function sendOtp(Request $request)
    {
        ...
        if (!$gare) {
            return response()->json([
                'success' => false,
                'message' => 'Aucun responsable de gare n\'existe avec cette adresse email.'
            ]);
        }
        ...
        try {
            Mail::send('emails.otp', ['otp' => $otpCode], function ($message) use ($email) {
                $message->to($email)->subject('Code de réinitialisation Gare - Car225');
            });
    ...
    public function resetPassword(Request $request)
    {
        ...
        Gare::where('email', $request->email)->update(['password' => Hash::make($request->password)]);
