...
class PasswordResetController extends Controller
{
    ...
    public function sendOtp(Request $request)
    {
        ...
        if (!$agent) {
            return response()->json([
                'success' => false,
                'message' => 'Aucun agent n\'existe avec cette adresse email.'
            ]);
        }
        ...
        try {
            Mail::send('emails.otp', ['otp' => $otpCode], function ($message) use ($email) {
                $message->to($email)->subject('Code de réinitialisation Agent - Car225');
            });
    ...
    public function resetPassword(Request $request)
    {
        ...
        Agent::where('email', $request->email)->update(['password' => Hash::make($request->password)]);
